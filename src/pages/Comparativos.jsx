...
function CustomTooltip({ active, payload, label, t }) {
  ...
  return (
    <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 8, padding: "8px 12px", boxShadow: t.shadow, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div style={{ fontSize: 13, fontFamily: "'Space Grotesk', sans-serif", color: t.primary }}>{formatCurrency(payload[0].value)}</div>
    </div>
  );
}
        ...
        <div style={{ width: "100%", height: 260 }}>
          <ResponsiveContainer>
            <BarChart data={porMes} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: t.textMuted, fontFamily: "'Space Grotesk', sans-serif" }} axisLine={false} tickLine={false} width={56} tickFormatter={(v) => `${(v / 1000).toFixed(v >= 1000 ? 0 : 1)}k`} />
